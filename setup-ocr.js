...
async function main() {
  try {
    console.clear();
    log.header("🚀 OCR Configuration Setup");
    console.log(
    ...
    if (fs.existsSync(envPath)) {
      const overwrite = await question(
        `${COLORS.yellow}⚠${COLORS.reset} .env.local already exists. Overwrite? (y/n) `
      );
    ...
    log.header("2️⃣  Backend API Configuration (Optional)");
...
`;
    ...
    // Check .gitignore
    log.header("3️⃣  Security Configuration");
