...
export default function UserStatements() {
  ...
  const handleBulkDownload = async (type = "zip") => {
    ...
    try {
      ...
      a.href = blobUrl;
      // name file according to requested type
      a.download = `account-statements-${new Date().getTime()}.${
        type === "zip" ? "zip" : "xlsx"
      }`;
      ...
      Swal.fire({
        icon: "error",
        title: backendMessage || "No files found for selected statements.",
        toast: true,
        position: "top-end",
        timer: 3000,
