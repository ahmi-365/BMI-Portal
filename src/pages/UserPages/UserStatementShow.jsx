...
export default function UserStatementShow() {
  ...
  const loadData = async () => {
    try {
      setIsLoading(true);
      const result = await userStatementsAPI.show(id);
      setData(result);
    } catch (error) {
  ...
  const handleDownload = async () => {
    try {
      const blob = await userStatementsAPI.download(id);
    ...
    return (
      <UserLayout>
        <div className="max-w-2xl mx-auto text-center py-12">
          ...
          <button
            onClick={() => navigate("/user/statements")}
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            {data.status && (
              <div>
                ...
                <p className="text-gray-900 dark:text-white">{data.status}</p>
              </div>
            )}
          </div>
