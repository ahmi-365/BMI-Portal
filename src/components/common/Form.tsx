...
export interface FormField {
  name: string;
  label: string;
  type: "text" | "email" | "password" | "number" | "date" | "select" | "textarea" | "toggle";
  placeholder?: string;
  required?: boolean;
  options?: SelectOption[];
  colSpan?: 1 | 2; // Controls width: 1 = half width (on desktop), 2 = full width
  defaultValue?: any;
}
  ...
  useEffect(() => {
    ...
    fields.forEach((field) => {
      if (initialData[field.name] === undefined) {
        initialData[field.name] = field.defaultValue ?? (field.type === "toggle" ? false : "");
      }
    });
              ...
              <div className="flex items-center justify-between rounded-lg border border-gray-200 p-4 dark:border-gray-700 dark:bg-gray-800/50">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                    formData[field.name] ? "bg-brand-500" : "bg-gray-200 dark:bg-gray-700"
                  }`}
                  role="switch"
                >
                  <span
